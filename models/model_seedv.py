...
class Model(nn.Module):
    ...
    def forward(self, x):
        bz, seq_len, ch_num, patch_size = x.shape  # [bs, 1/6, 62, 200]
        x = x.contiguous().view(bz * seq_len, ch_num, 1, 200)
        epoch_features = self.backbone(x)
        epoch_features = epoch_features.contiguous().view(bz, seq_len, ch_num * 1 * 200)
