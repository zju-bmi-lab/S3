...
class Model(nn.Module):
    ...
    def forward(self, x):
        bz, seq_len, ch_num, patch_size = x.shape  # [bs, 5, 16, 1000]
        x = x.contiguous().view(bz * seq_len, ch_num, 5, 200)
        epoch_features = self.backbone(x)
        epoch_features = epoch_features.contiguous().view(bz, seq_len, ch_num * 5 * 200)
